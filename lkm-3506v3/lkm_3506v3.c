#include "lkm_3506v3.h"

#include <string.h>

#define LKM_3506V3_CMD_DISABLE             0x80U
#define LKM_3506V3_CMD_STOP                0x81U
#define LKM_3506V3_CMD_ENABLE              0x88U
#define LKM_3506V3_CMD_READ_ENCODER        0x90U
#define LKM_3506V3_CMD_READ_STATE_2        0x9CU
#define LKM_3506V3_CMD_REL_POSITION_SPEED  0xA8U
...
static int32_t wrapped_delta(uint16_t from, uint16_t to)
{
    ...
    if (delta > (int32_t)LKM_3506V3_ENCODER_COUNTS_PER_REV / 2) {
        delta -= LKM_3506V3_ENCODER_COUNTS_PER_REV;
    } else if (delta < -(int32_t)LKM_3506V3_ENCODER_COUNTS_PER_REV / 2) {
        delta += LKM_3506V3_ENCODER_COUNTS_PER_REV;
    }
    return delta;
}

static int send_command(struct lkm_3506v3 *motor, uint8_t command)
{
    uint8_t data[LKM_3506V3_CAN_FRAME_SIZE] = {0};
    if (motor == NULL || motor->config.send == NULL) {
        return LKM_3506V3_ERROR_ARGUMENT;
    }
    data[0] = command;
                             ...
                             void *send_user)
{
    ...
    *config = (struct lkm_3506v3_config) {
        .can_id = LKM_3506V3_DEFAULT_CAN_ID,
        .mechanical_min_deg = 0.0f,
        .mechanical_max_deg = 345.0f,
        .center_raw = 12178U,
        .encoder_direction = 1,
        .counts_per_deg = 91.8087f,
        .center_deg = 172.5f,
        .send = send,
        .send_user = send_user
    };
                  ...
                  const struct lkm_3506v3_config *config)
{
    ...
    motor->config = *config;
    return LKM_3506V3_OK;
}

int lkm_3506v3_enable(struct lkm_3506v3 *motor) { return send_command(motor, LKM_3506V3_CMD_ENABLE); }
int lkm_3506v3_disable(struct lkm_3506v3 *motor) { return send_command(motor, LKM_3506V3_CMD_DISABLE); }
int lkm_3506v3_stop(struct lkm_3506v3 *motor) { return send_command(motor, LKM_3506V3_CMD_STOP); }
int lkm_3506v3_read_encoder(struct lkm_3506v3 *motor) { return send_command(motor, LKM_3506V3_CMD_READ_ENCODER); }
int lkm_3506v3_read_state(struct lkm_3506v3 *motor) { return send_command(motor, LKM_3506V3_CMD_READ_STATE_2); }
                          ...
                          uint16_t max_speed_dps)
{
    ...
    if (target_deg < motor->config.mechanical_min_deg ||
        ...
        return LKM_3506V3_ERROR_RANGE;
    }
    if (!motor->feedback_valid) {
        return LKM_3506V3_ERROR_NOT_READY;
    }
    ...
    data[0] = LKM_3506V3_CMD_REL_POSITION_SPEED;
