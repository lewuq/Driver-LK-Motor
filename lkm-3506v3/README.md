# MS3506V3 Portable CAN Driver

**🌐 [English](README.md) | [中文](README_CN.md)**

MS3506V3 Portable CAN Driver is a small, board-independent C interface for the
15-bit classic-CAN version of the LK Motor MS3506V3. It builds and parses the
...
| Item | Default |
| --- | --- |
| Motor | MS3506V3 CAN, 15-bit encoder |
| CAN ID | `0x141` |
| Bus format | Classic CAN, standard ID, 8 data bytes |
| Typical bitrate | 1 Mbit/s, configured by the application |
| Mechanical range | `0..345°` |
| Calibrated center | raw `12178` at `172.5°` |
| Scale | `91.8087 counts/°` |
...
Copy `lkm_3506v3.c` and `lkm_3506v3.h` into the target firmware. Implement one
...
struct lkm_3506v3 motor;
struct lkm_3506v3_config config;

lkm_3506v3_default_config(&config, board_can_send, can_handle);
lkm_3506v3_init(&motor, &config);
lkm_3506v3_read_encoder(&motor);
```
...
```c
lkm_3506v3_process_frame(&motor, rx_id, rx_data, rx_length, uptime_ms);
```

After a valid `0x90` reply, `motor.angle_deg` and the encoder fields are
available. `lkm_3506v3_set_position()` requires this feedback before it can turn a
...
| Function | Purpose |
| --- | --- |
| `lkm_3506v3_default_config()` | Load tested defaults and attach the send callback. |
| `lkm_3506v3_init()` | Validate configuration and initialize runtime state. |
| `lkm_3506v3_enable()` / `disable()` / `stop()` | Build and send basic motor commands. |
| `lkm_3506v3_read_encoder()` | Request 15-bit raw position with command `0x90`. |
| `lkm_3506v3_read_state()` | Request state feedback with command `0x9C`. |
| `lkm_3506v3_set_position()` | Send a bounded target as a relative `0xA8` move. |
| `lkm_3506v3_process_frame()` | Parse a received frame and update motor state. |
| `lkm_3506v3_feedback_fresh()` | Check application-provided feedback timestamps. |
